      ...
      if(FD_ISSET(i,&tmp_set)){
        ...
        if(ntohs(recvaddr.sll_protocol) == ETH_P_ARP) {
          ...
          if(ipHeader.protocol == IPPROTO_ICMP) {
            ...
            memcpy(&icmp_hdr, buf + sizeof(struct ether_header) + sizeof(struct iphdr), sizeof(struct icmphdr));
...
void process_arp(char *buf, int rec, (struct sockaddr_ll) socket){
  ...
  arp_hdr arp_header;
  memcpy(&arp_header,buf+sizeof(struct ether_header),sizeof(arp_hdr));


  //	struct ether_header eth_hdr_tmp;
  //	arp_hdr arp_header_tmp;

  if(arp_header.opcode == ntohs(ARPOP_REPLY)){
    return;
  }
  ...
  int i;
  for(i=0;i<4;i++){
    tmp_paddr = arp_header.sender_paddr[i];
    arp_header.sender_paddr[i] = arp_header.target_paddr[i];
    arp_header.target_paddr[i] = tmp_paddr;
  }

  // change sender mac to target mac
  for(i=0;i<6;i++){
    arp_header.target_haddr[i] = arp_header.sender_haddr[i];
  }

  // fill in our mac as sender
  memcpy(arp_header.sender_haddr, socket.sll_addr, 6);
  // uint8_t tmp_haddr;
  // for(i=0;i<6;i++){
  //   arp_header.sender_haddr[i] = tmpIface->haddr[i]; //IGNOREME
  // 	 tmp_haddr = arp_header.sender_haddr[i];
  //   arp_header.sender_haddr[i] = arp_header.target_haddr[i];
  //   arp_header.target_haddr[i] = tmp_haddr;
  // }

  uint8_t tmp_ehdr;
  for(i=0;i<6;i++) {
    //eth_hdr.ether_dhost[i] = eth_hdr.ether_shost[i];
    //eth_hdr.ether_shost[i] = tmpIface->haddr[i];  //IGNOREME
    tmp_ehdr = eth_hdr.ether_dhost[i];
    eth_hdr.ether_dhost[i] = eth_hdr.ether_shost[i];
    eth_hdr.ether_shost[i] = tmp_ehdr;
  }

  memcpy(buf,&eth_hdr,sizeof(struct ether_header));
  memcpy(buf+sizeof(struct ether_header),&arp_header,sizeof(arp_hdr));
